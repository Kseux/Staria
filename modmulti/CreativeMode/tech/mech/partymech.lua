...
function update(args)
  ...
  local mechProjectile2 = tech.parameter("mechProjectile2")
  local mechProjectileCost = tech.parameter("mechProjectileCost")
  local mechProjectile2Cost = tech.parameter("mechProjectile2Cost")  
  local mechProjectileConfig = tech.parameter("mechProjectileConfig")
  local mechProjectile2Config = tech.parameter("mechProjectile2Config")
  ...
  if data.active then
    ...
    if args.actions["mechFire"] then
      if data.fireTimer <= 0 and cost + mechProjectileCost <= args.availableEnergy then
		...
		cost = cost + mechProjectileCost
        tech.setAnimationState("frontFiring", "fire")
      else
        local oldFireTimer = data.fireTimer
        data.fireTimer = data.fireTimer - args.dt
        if oldFireTimer > mechFireCycle / 2 and data.fireTimer <= mechFireCycle / 2 and cost + mechProjectileCost <= args.availableEnergy then
	...
	if args.actions["mechFire2"] then
      if data.fireTimer <= 0 and cost + mechProjectile2Cost <= args.availableEnergy then
        world.spawnProjectile(mechProjectile2, tech.anchorPoint("topfrontGunFirePoint"), tech.parentEntityId(), {math.cos(aimAngle), math.sin(aimAngle)}, false, mechProjectile2Config)
        data.fireTimer = data.fireTimer + mechFireCycle
		cost = cost + mechProjectile2Cost
        tech.setAnimationState("frontFiring", "fire")
      else
        local oldFireTimer = data.fireTimer
        data.fireTimer = data.fireTimer - args.dt
        if oldFireTimer > mechFireCycle / 2 and data.fireTimer <= mechFireCycle / 2 and cost + mechProjectile2Cost <= args.availableEnergy then
          world.spawnProjectile(mechProjectile2, tech.anchorPoint("topbackGunFirePoint"), tech.parentEntityId(), {math.cos(aimAngle), math.sin(aimAngle)}, false, mechProjectile2Config)
		  cost = cost + mechProjectile2Cost
          tech.setAnimationState("backFiring", "fire")
        end
      end
